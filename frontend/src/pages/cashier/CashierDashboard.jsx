...
if (e.code.startsWith("Numpad")) {
  ...
  /* =========================
     MANUAL / LASER BARCODE
     ========================= */
  const handleManualAdd = async (codeToScan = barcodeInput) => {
    if (!codeToScan) return;
    const product = await getProductByBarcode(codeToScan);
    if (!product || product?.success === false) {
      speak("Product not found");
      return;
    }
  ...
  return (
    <div className="min-h-screen grid grid-cols-1 lg:grid-cols-3 bg-gray-100">
      <div className="lg:col-span-2 p-4 space-y-4">
        <div className="flex flex-wrap gap-2 bg-white p-3 rounded-2xl shadow">
          ...
          <input
            ...
            placeholder="Barcode"
          />
          <button onClick={() => handleManualAdd()} className="bg-green-600 text-white px-4 py-2 rounded-xl">
            Add
          </button>
        </div>
        {scanning && (
      ...
      <div ref={receiptRef} className="bg-white p-4 shadow-lg flex flex-col">
        ...
        <div className="border-t pt-4 mt-4">
          <h2 className="text-3xl font-black">TOTAL: {total.toFixed(2)} €</h2>
          <button
            disabled={cart.length === 0}
            onClick={() => setShowPayment(true)}
            className={`w-full py-3 mt-3 rounded-2xl font-bold ${cart.length === 0 ? "bg-gray-300" : "bg-green-600 text-white"}`}
          >
            Checkout
          </button>
          <button
            disabled={cart.length === 0}
            onClick={payCash}
            className={`w-full py-3 mt-2 rounded-2xl font-bold ${cart.length === 0 ? "bg-gray-300" : "bg-black text-white"}`}
          >
            Pay Cash
          </button>
        </div>
      ...
      {showPayment && (
        <SplitPaymentModal total={total} onClose={() => setShowPayment(false)} onPay={handlePayment} />
      )}
    </div>
  );
}
