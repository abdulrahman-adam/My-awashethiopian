...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    // Only scroll if a section is active
    if (activeSection) {
      setTimeout(() => {
        sectionRef.current?.scrollIntoView({
          behavior: "smooth",
  ...
  return (
    <div className="px-2 md:px-4 lg:px-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-6 mt-8">
        {[
          { title: "User Management", section: "users", icon: Users, color: "text-blue-600" },
          { title: "Category Management", section: "categories", icon: Tags, color: "text-orange-600" },
          { title: "Product Management", section: "products", icon: Package, color: "text-indigo-600" },
          { title: "Reports", section: "reports", icon: TrendingUp, color: "text-green-600" },
          { title: "Database", section: "database", icon: Database, color: "text-purple-600" },
        ].map((card, i) => (
          <div
            key={i}
            onClick={() => setActiveSection(card.section)}
            className={`bg-white p-6 rounded-xl shadow-sm border cursor-pointer transition-all duration-300 hover:shadow-lg ${
              activeSection === card.section ? "border-blue-500 ring-2 ring-blue-100" : "border-gray-100"
            }`}
          >
      ...
      {/* PERSISTENT SECTIONS (Always mounted, hidden via CSS) */}
      <div ref={sectionRef} className={`${activeSection ? "block" : "hidden"} mt-8`}>
        {activeSection && (
          <button
            onClick={() => setActiveSection("")}
            className="mb-4 px-4 py-2 text-sm bg-gray-100 hover:bg-gray-200 rounded-lg transition"
        ...
        <div className={activeSection === "users" ? "block" : "hidden"}> <UserManagement /> </div>
        <div className={activeSection === "categories" ? "block" : "hidden"}> <CategoryManagement /> </div>
        <div className={activeSection === "products" ? "block" : "hidden"}> <ProductManagement /> </div>
        <div className={activeSection === "reports" ? "block" : "hidden"}> <ReportsDashboard /> </div>
        <div className={activeSection === "database" ? "block" : "hidden"}> <InventoryDashboard /> </div>
      </div>

      {/* DEFAULT VIEW */}
      {!activeSection && (
        <div className="w-full bg-white py-6 shadow-sm mt-8 px-4 text-center">
          <p className="text-gray-400">📊 Select a module to manage system data</p>
        </div>
      )}
      ...
      {showRegister && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white shadow-2xl rounded-2xl w-full max-w-2xl max-h-[95vh] flex flex-col">
            <div className="flex justify-between border-b p-5">
              <h2 className="text-xl font-bold">Create New User</h2>
              <button onClick={() => setShowRegister(false)} className="text-red-600 font-bold">✕</button>
            </div>
            <div className="p-4 overflow-auto"><Register /></div>
          </div>
        </div>
      )}
    </div>
